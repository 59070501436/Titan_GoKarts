  ...
  max_rpm(0)
{
  ...
  nhLocal.param("encoder_velocity_ppr", encoder_velocity_ppr, 12);
  ROS_INFO_STREAM("encoder_velocity_ppr: " << encoder_velocity_ppr);
  nhLocal.param("encoder_steering_ppr", encoder_steering_ppr, 500);
...
// #endif
  ...
  {
    // motor speed (rpm)
    int32_t right_rpm = 500*steering;
    int32_t left_rpm =  100000*velocity;
 #ifdef _CMDVEL_DEBUG
...
void MainNode::cmdvel_setup()
{
  ...
  controller.write("^KP 1 \r");
  controller.write("^KP 2 20\r");
  controller.write("^KI 1 10\r");
  ...
  right_enccmd << "^EPPR 1 " << encoder_velocity_ppr << "\r";
  left_enccmd << "^EPPR 2 " << encoder_steering_ppr << "\r";
